...
#define FT_CALL(c, r) { int err = c; if(err != 0) { _LogFTError(__FILE__, __LINE__, __FUNCTION__, LOG_CATEGORY, err); return r; } }
#define FT_CALL_NORETURN(c) { int err = c; if(err != 0) _LogFTError(__FILE__, __LINE__, __FUNCTION__, LOG_CATEGORY, err); }
