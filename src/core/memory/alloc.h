...
#define salloc(size) _salloc(__FILE__, __LINE__, __FUNCTION__, size)
#define msalloc(size) _salloc(__FILE__, __LINE__, __FUNCTION__, sizeof(size))

// Allocates memory with calloc, with error logging.
#define scalloc(nmemb, size) _scalloc(__FILE__, __LINE__, __FUNCTION__, nmemb, size)
#define mscalloc(nmemb, size) _scalloc(__FILE__, __LINE__, __FUNCTION__, nmemb, sizeof(size))

// Reallocates memory with realloc, with error logging.
#define resalloc(ptr, size) _resalloc(__FILE__, __LINE__, __FUNCTION__, ptr, size)

// Frees memory with free, with error logging.
// Also sets ptr to NULL, so that it's harder to double-free.
#define sfree(ptr) { _sfree(__FILE__, __LINE__, __FUNCTION__, ptr); ptr = NULL; }
