...
#define render_quad(camera, transform, tex, use_dims) render_quad_color(camera, transform, tex, use_dims, (vec4){.x=1,.y=1,.z=1,.w=1})
bool render_quad_untex_color(mat4 camera, mat4 transform, vec4 color);
#define render_quad_untex(camera, transform) render_quad_untex_color(camera, transform, (vec4){.x=1,.y=1,.z=1,.w=1})
bool render_quad_subtex_color(mat4 camera, mat4 transform, texture* tex, bool use_dims, vec4 uv_rect, vec4 color);
#define render_quad_subtex(camera, transform, tex, use_dims, uv_rect) render_quad_subtex_color(camera, transform, tex, use_dims, uv_rect, (vec4){.x=1,.y=1,.z=1,.w=1})
bool render_text_color(mat4 camera, mat4 transform, font* ft, int glyph, bool use_dims, vec4 color);
#define render_text(camera, transform, ft, glyph, use_dims) render_text_color(camera, transform, ft, glyph, use_dims, (vec4){.x=1,.y=1,.z=1,.w=1})
bool render_text_string_color(mat4 camera, mat4 transform, text* txt, vec4 color);
#define render_text_string(camera, transform, txt) render_text_string_color(camera, transform, txt, (vec4){.x=1,.y=1,.z=1,.w=1})
